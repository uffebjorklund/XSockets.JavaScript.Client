 ...
 */
class xsocketsClient implements itransport {
        ...
        this.socket.onmessage = (event: MessageEvent) => {
            ...
            if (typeof event.data === "string") {
                ...
                if (d.T == xsockets.events.authfailed) {
                    this.onAuthenticationFailed(d.D);
                    this.close(false);
                    return;
                }

                var c = this.controller(d.C, false) as controller;
                if (c == undefined) {
                    this.onMessage(d);
                }
                else {
                ...
                var c = this.controller(bd.C, false) as controller;
                if (c == undefined) {
     ...
     */
    public disposeController(controller: controller) {
